...
# Force-import the *installed* lpips package, not the local lpips/ directory.
# train.py already loaded the local one into sys.modules, so we must evict it.
_project_dir = os.path.dirname(os.path.abspath(__file__))
_path_backup = sys.path.copy()
sys.path = [p for p in sys.path if os.path.abspath(p) != _project_dir]
_saved_lpips = {k: sys.modules.pop(k) for k in list(sys.modules) if k == 'lpips' or k.startswith('lpips.')}
import lpips
sys.path = _path_backup
sys.modules.update(_saved_lpips)
